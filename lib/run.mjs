...
export function run(doc, scenario) {
  ...
  // `kind` says why a run stopped where something wants to tell the two apart: `unmatched` is a
  // node no branch took the run out of, `ambiguous` one where more than one did, and `edges`
  // names those branches.
  const fail = (message, at, more = {}) => { result.error = { message, at, ...more }; return result; };
  let inputs;
  ...
  for (let i = 0; i < MAX_STEPS; i++) {
    ...
    else if (matched.length === 0) return fail(`nothing matched leaving "${node.label}"`, node.id, { kind: 'unmatched' });
    else return fail(`ambiguous: ${matched.length} branches match leaving "${node.label}"`, node.id, { kind: 'ambiguous', edges: matched.map((e) => e.id) });
...
export function lint(doc) {
  ...
  for (const n of doc.nodes ?? []) {
    ...
    // A guard needs something to choose between, and what supplies that is a fork — more than one
    // way out — not the kind of node the fork leaves. With one way out a condition cannot send the
    // run anywhere: when it holds the run carries on as it would anyway, and when it does not the
    // run stops with nothing matched. That is true of a lone branch off a decision as much as of a
    // condition left behind on an action, so both are named here and the kind is not consulted.
    // An end is left out: that it has a way out at all is the problem there, and it is named above.
    if (n.kind !== 'end' && edges.length === 1) {
      for (const e of edges) {
        if (e.else) problems.push({ message: `"${n.label}" has one way out, so "else" on it has no other branch to fall through from`, edge: e.id });
        else if (e.when && e.when.trim()) problems.push({ message: `"${n.label}" has one way out, so the condition on it cannot branch, it can only stop the run; draw its other branch, or empty the condition`, edge: e.id });
      }
    }
    // A decision does nothing but choose, so one that never chooses is a node with no purpose. Two
    // or more ways out are caught above when they cannot all be told apart; this is the other end.
    if (n.kind === 'decision' && edges.length === 1 && guarded.length === 0) problems.push({ message: `"${n.label}" is a decision with one unguarded way out; it is not choosing between anything`, node: n.id });
    if (n.kind === 'decision' && edges.length > 1 && guarded.length === 0) problems.push({ message: `"${n.label}" is a decision with no condition on any branch`, node: n.id });
