...
import { test, check, compile, evaluate, same, names, truthy } from './expr.mjs';
...
export function initialState(doc, inputs = null) {
  ...
  for (const f of doc.state ?? []) {
    ...
    if (inputs && typeof v === 'string' && v.trim()) {
      try { const ast = compile(v); if (names(ast).every((n) => known.has(n.split('.')[0]))) v = evaluate(ast, { inputs, state: {}, enums: enumsOf(doc) }); }
      catch { /* not an expression: the text itself is the value */ }
    }
    s[f.name] = v;
  }
  return s;
...
/** Whether an initial value would be read as an expression rather than taken as it is written. */
export function initialIsExpression(doc, src) {
  if (typeof src !== 'string' || !src.trim()) return false;
  const known = enumsOf(doc);
  for (const i of doc.inputs ?? []) known.add(i.name);
  try { return names(compile(src)).every((n) => known.has(n.split('.')[0])); } catch { return false; }
}
