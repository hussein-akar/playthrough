// The flow as a Markdown spec: the same drawing and spreadsheet, but pasteable into a ticket, a
// wiki page or a pull request. Nothing here reads the DOM; the results come from `runAll`, so the
// pass/fail column is the one the page shows.
import { runAll } from './run.mjs';

const cell = (v) => (v == null ? '' : String(v)).replace(/\|/g, '\\|').replace(/\r?\n/g, ' ').trim();
const code = (v) => { const s = cell(v); return s ? `\`${s}\`` : ''; };
const table = (head, rows) => [`| ${head.join(' | ')} |`, `|${head.map(() => '---').join('|')}|`, ...rows.map((r) => `| ${r.join(' | ')} |`)].join('\n');
const fmt = (v) => (v == null || v === '' ? '' : typeof v === 'string' ? v : JSON.stringify(v));

export function toMarkdown(doc, results = runAll(doc)) {
  const out = [`# ${cell(doc.name) || 'Untitled flow'}`];
  if (doc.description?.trim()) out.push('', doc.description.trim());

  const inputs = doc.inputs ?? [], state = doc.state ?? [];
  out.push('', '## Inputs', '');
  out.push(inputs.length
    ? table(['Name', 'Type', 'Values'], inputs.map((i) => [code(i.name), cell(i.type), i.type === 'enum' ? (i.values ?? []).map(code).join(', ') : i.type === 'list' ? (i.fields ?? []).map((f) => `${code(f.name)} ${cell(f.type)}${f.type === 'enum' ? ` (${(f.values ?? []).map(cell).join(', ')})` : ''}`).join('; ') : '']))
    : '_None._');
  out.push('', '## State', '');
  out.push(state.length
    ? table(['Field', 'Initial'], state.map((f) => [code(f.name), code(fmt(f.initial) || 'null')]))
    : '_None._');

  const nodes = new Map((doc.nodes ?? []).map((n) => [n.id, n]));
  // Every fork in the drawing, which is every node with more than one way out: a decision, and any
  // action that branches on what it just did. A decision is listed even when it has yet to fork, so
  // a half-drawn one is not quietly missing from the spec.
  const forks = (doc.nodes ?? []).filter((n) => n.kind === 'decision' || (doc.edges ?? []).filter((e) => e.from === n.id).length > 1);
  out.push('', '## Branches', '');
  if (!forks.length) out.push('_None._');
  for (const d of forks) {
    out.push(`- **${cell(d.label) || d.id}**`);
    for (const e of (doc.edges ?? []).filter((e) => e.from === d.id)) {
      const to = nodes.get(e.to)?.label ?? e.to;
      const guard = e.else ? '_else_' : e.when?.trim() ? code(e.when) : '_(unguarded)_';
      out.push(`  - ${e.label?.trim() ? `${cell(e.label)} · ${guard}` : guard} → ${cell(to)}`);
    }
  }

  const rs = results?.results ?? [];
  out.push('', '## Scenarios', '');
  if (!rs.length) out.push('_None._');
  else {
    out.push(`${results.passed} of ${rs.length} pass.`, '');
    const tagged = rs.some(({ scenario: s }) => s.tags?.length);
    const head = ['Scenario', ...(tagged ? ['Tags'] : []), ...inputs.map((i) => code(i.name)), 'Expected actions', 'Lands on', ...state.map((f) => code(f.name)), 'Result'];
    const rows = rs.map(({ scenario: s, verdict: v }) => [
      cell(s.name),
      ...(tagged ? [(s.tags ?? []).map(code).join(' ')] : []),
      ...inputs.map((i) => cell(fmt(s.inputs?.[i.name]).replace(/\r?\n/g, '; '))),
      (s.expect?.actions ?? []).map(cell).filter(Boolean).join(', '),
      cell(s.expect?.end),
      ...state.map((f) => cell(fmt(s.expect?.state?.[f.name]))),
      v.pass ? '✅ pass' : `❌ fail: ${v.issues.map((i) => cell(i.message)).join('; ')}`,
    ]);
    out.push(table(head, rows));
    // The same rows tallied by tag: "every edge case passes" is a sentence a reviewer looks for.
    const byTag = new Map();
    for (const { scenario: s, verdict: v } of rs) for (const t of s.tags ?? []) { const e = byTag.get(t) ?? { total: 0, passed: 0 }; e.total++; if (v.pass) e.passed++; byTag.set(t, e); }
    if (byTag.size) { out.push('', '### By tag', ''); for (const [t, e] of byTag) out.push(`- ${code(t)} · ${e.passed} of ${e.total} pass`); }
  }
  return out.join('\n') + '\n';
}
