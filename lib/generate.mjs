...
const root = (a) => (a?.op === 'where' ? root(a.left) : a?.op === 'method' || a?.op === 'get' ? root(a.recv) : a?.op === 'name' ? a.name.split('.')[0] : null);
...
function facts(doc) {
  ...
  /**
   * Inside `o -> …` the record is `o` and only `o`, so `o.qty` is the line's `qty` and a bare `qty`
   * is the input of that name. Both come out as plain names, so each is marked with which it was:
   * `rec` true is a field of the list in scope, false is a name from outside it.
   */
  function strip(a, param) {
    if (!a || typeof a !== 'object') return a;
    if (a.op === 'name') return a.name.split('.')[0] === param ? { ...a, name: a.name.slice(param.length + 1) || param, rec: true } : { ...a, rec: false };
    const out = { ...a };
    for (const k of ['left', 'right', 'value', 'recv', 'body']) if (out[k]) out[k] = strip(out[k], param);
    for (const k of ['items', 'args']) if (out[k]) out[k] = out[k].map((it) => strip(it, param));
    return out;
  }
  function walk(a, ctx, bool, touch) {
    ...
    switch (a.op) {
      case 'lit': return;
      case 'name': { const f = mention(a.name, a.rec === false ? null : ctx, touch); if (f && bool) f.truth = true; return; }
      case '==': case '!=': case '<': case '<=': case '>': case '>=': {
        const { left: l, right: r } = a;
        const at = (x) => (x.rec === false ? null : ctx);
        const fl = l.op === 'name' ? factsOf(target(l.name, at(l))) : null, fr = r.op === 'name' ? factsOf(target(r.name, at(r))) : null;
        ...
        for (const side of [l, r]) if (side.op === 'name') mention(side.name, at(side), touch); else walk(side, ctx, false, touch);
        return;
      }
      case 'in': {
        const f = a.left.op === 'name' ? factsOf(target(a.left.name, a.left.rec === false ? null : ctx)) : null;
      ...
      // `lines.count(o -> o.qty > 1)`: the condition is about one record of the receiver, the same
      // way a `where` is, and `o` names that record, so a fact about `o.qty` is a fact about `qty`.
      case 'method': { const lc = listCtx(a.recv); walk(a.recv, ctx, false, touch); for (const arg of a.args) walk(arg, lc ?? ctx, true, touch); return; }
      case 'lambda': walk(strip(a.body, a.param), ctx, bool, touch); return;
      case 'get': walk(a.recv, ctx, false, touch); return;
      case 'call': for (const arg of a.args) walk(arg, ctx, false, touch); return;
  ...
  for (const s of doc.state ?? []) {
    ...
    const above = [...state.keys()];   // an initial is read over the state fields declared before it
    state.set(s.name, st);
    if (!initialIsExpression(doc, s.initial, above)) continue;
