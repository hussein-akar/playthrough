...
import { readFile, writeFile, readdir, stat, unlink, mkdir, rename, rmdir } from 'node:fs/promises';
import { basename, join, resolve, dirname } from 'node:path';
import { runAll, lint } from './run.mjs';

const SEG = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;   // one path segment: no dot-files, nothing that walks the tree
const MAX_DEPTH = 8;
...
/** Remove a folder, only when nothing is in it (`code: 'NOTEMPTY'` otherwise). */
export async function deleteFolder(dir, path) {
  if (!isFolderPath(path)) throw badName(path);
