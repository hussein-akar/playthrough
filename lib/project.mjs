 ...
 * names the folders on the way, kept as typed: `Billing / Refund intake` → `Billing/refund-intake.json`.
 */
const slug = (x) => String(x || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
/** A group's folder name is kept as typed, `Sales Orders`, less anything a path segment cannot hold. */
...
export function fileFor(name) {
  ...
  const parts = segs.slice(0, -1).map(folderName).filter(Boolean).concat(slug(segs.at(-1)) || []);
