...
/**
 * Give a flow's file another name, made from `name` the way a new file's is. Refuses to land on
 * a file that exists (`code: 'EXISTS'`). Resolves to the new file name and its mtime, which the
 * rename leaves as it was, so a page holding the old one can still save without a false conflict.
 */
export async function renameFlow(dir, from, name) {
  if (!isFlowFile(from)) throw badName(from);
  const to = fileFor(name);
