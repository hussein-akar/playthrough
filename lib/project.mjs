...
/** A file or folder name is kept as typed, `Sales Orders`, less anything a path segment cannot hold. */
export const folderName = (x) => String(x ?? '').replace(/[^A-Za-z0-9 ._-]+/g, ' ').replace(/\s+/g, ' ').trim().replace(/^[^A-Za-z0-9]+/, '');

/**
 * A file name for a flow called `name`: the name as typed, `.json` at the end. A name with slashes
 * names the folders on the way: `Billing / Refund intake` → `Billing/Refund intake.json`.
 */
export function fileFor(name) {
  const parts = String(name || '').split('/').map(folderName).filter(Boolean);
...
/**
 * Give a flow's file another name, made from `name` the way a new file's is (as typed), in the folder it
...
export async function renameFlow(dir, from, name, folder = null) {
  ...
  if (!sameButCase(from, to) && await stat(join(dir, to)).then(() => true, () => false)) throw Object.assign(new Error(`${to} already exists`), { code: 'EXISTS' });
...
/**
 * Give a group another name, kept as typed, in the parent it is in,
 * or under `folder` when one is given ('' for the root); a name with slashes spells out the groups
 * it moves under instead. Everything in it comes along. Refuses to land on a folder that exists
 * (`code: 'EXISTS'`) or inside itself. Resolves to the new path.
 */
export async function renameFolder(dir, from, name, folder = null) {
  if (!isFolderPath(from)) throw badName(from);
  const parts = String(name ?? '').split('/').map(folderName).filter(Boolean);
  const to = String(name ?? '').includes('/') ? parts.join('/') : under(folder ?? folderOf(from), parts.join('/'));
...
/** `advanced` → `Advanced`: on a disk that ignores case the target "exists", but it is the same entry, and rename changes its case. */
const sameButCase = (a, b) => a !== b && a.toLowerCase() === b.toLowerCase();
