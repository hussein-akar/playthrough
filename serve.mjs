 ...
 * DELETE /api/folders/:path (?all=1 takes everything in it). A path inside the project travels URL-encoded, slashes included.
 */
async function api(req, res, url) {
  ...
  try {
    ...
    if (what === 'folders' && file && req.method === 'DELETE') return send(res, 200, await deleteFolder(project.dir, file, { all: url.searchParams.get('all') === '1' }));
